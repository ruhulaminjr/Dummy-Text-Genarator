...
const showParagraph = document.querySelector('#result');

// Event listener

form.addEventListener('submit',textGenarator);

// function 

function textGenarator(e){
  e.preventDefault();
  ...
  if (isNaN(numberValue) || numberValue <= 0 || numberValue > 9 ){
    showParagraph.innerHTML = `<p>${dummyText[random]}</p>`;
  }
  else{
    let cutArray = dummyText.slice(0,numberValue);
    let ptext = cutArray.map(mergetext => ( `<p>${mergetext}</p>`)).join('');
    showParagraph.innerHTML = ptext;
  }
